...
def batch_backfill_sql(total_rows)
  ...
  (0..total_rows).step(BATCH_SIZE) do |offset|
    sql = <<~SQL
      WITH rows AS
        (SELECT id FROM load_test ORDER BY id
         LIMIT #{BATCH_SIZE} OFFSET #{offset})
      UPDATE load_test SET username = concat('Name_', md5(random()::text))
      WHERE EXISTS
        (SELECT * FROM rows WHERE load_test.id = rows.id);
    SQL
    statements.push(sql)
  end
  statements
...
def insert_ids_and_backfill(testing_size)
  ...
  # db_conn.exec_params(backfill_no_batching)
ensure
  db_conn&.close
end
...
testing_size = 10**7
insert_ids_and_backfill(testing_size)
